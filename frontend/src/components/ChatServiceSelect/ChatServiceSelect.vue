...
const { isShowChatServiceSelectModal, sydneyConfigs, selectedSydneyBaseUrl } =
...
const resetSydneyConfigs = () => {
  chatStore.resetSydneyConfigs();
};
...
<template>
  <NModal
    ...
    v-model:show="isShowChatServiceSelectModal"
    preset="card"
    title="聊天服务器设置"
  >
    <NTable striped>
      <tbody>
        <tr v-for="(config, index) in sydneyConfigs" :key="index">
          <td>
            <span v-if="config.isCus" class="hidden lg:block">{{
              config.label
            }}</span>
            ...
            <NInput
              ...
              @change="handleChangeSydneyConfig(config)"
            ></NInput>
          </td>
          ...
          <td>
            ...
            <div
              v-else-if="config.baseUrl"
              class="flex justify-center items-center flex-wrap gap-2"
              @click="checkSydneyConfig(config)"
            >
              <NTag
                v-if="config.isUsable === false"
                type="error"
                class="cursor-pointer"
                >不可用</NTag
              >
              <NTag v-if="config.delay" type="success" class="cursor-pointer"
                >{{ config.delay }} ms</NTag
              >
            </div>
          </td>
          <td>
            <div class="flex justify-center items-center flex-wrap gap-2">
              ...
              >
              <NButton
                v-if="config.baseUrl === selectedSydneyBaseUrl"
                secondary
                type="success"
                >当前</NButton
              >
              <NButton
                ...
                secondary
                type="info"
                @click="selectSydneyConfig(config)"
                >选择</NButton
              >
            </div>
          </td>
    ...
    <template #action>
      <NButton ghost size="large" type="info" @click="resetSydneyConfigs"
        >重置</NButton
      >
    </template>
  </NModal>
